...
export function generateRedactedPDF(options = {}) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Process redacted text with highlighting
      doc.fontSize(10).font('Helvetica');

      // Split text into segments and highlight redactions
      const segments = parseRedactedText(redactedText);

      for (const segment of segments) {
        if (segment.isRedaction) {
          // Redacted text with black box styling
          doc
            .fillColor('#000000')
            .rect(doc.x, doc.y, doc.widthOfString(segment.text) + 4, 14)
            .fill();

          doc
            .fillColor('#ffffff')
            .text(segment.text, { continued: segments.indexOf(segment) < segments.length - 1 });

          doc.fillColor('#000000');
        } else {
          // Normal text
          const continued = segments.indexOf(segment) < segments.length - 1;
          doc.text(segment.text, { continued });
        }
      }
